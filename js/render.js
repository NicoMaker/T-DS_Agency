/* ============================================
   render.js — Popola DOM da dati JSON:
               progetti (+ filtri + link sito),
               servizi, contatti, footer social
   ============================================ */

/* SVG icona "apri link esterno" */
const SVG_EXTERNAL = `<svg viewBox="0 0 24 24"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>`;
...
export function renderProgetti(progettiData, revealObserver) {
  ...
  /* 1. Categorie uniche dal JSON — ordine di apparizione */
  const categorie = ['Tutti', ...new Set(progetti.map(p => p.categoria))];

  /* 2. Filter buttons */
  filterBar.innerHTML = categorie.map(cat => `
    <button class="filter-btn${cat === 'Tutti' ? ' active' : ''}" data-cat="${cat}">
      ${cat}
    </button>
  `).join('');
  ...
  grid.innerHTML = progetti.map(p => `
    <div class="project-card reveal" data-cat="${p.categoria}">
      <div class="project-img-wrap">
        ...
        <!-- Badge categoria: visibile su "Tutti", nascosto su filtro singolo -->
  ...
  filterBar.addEventListener('click', e => {
    ...
    grid.querySelectorAll('.project-card').forEach(card => {
      ...
      /* Badge categoria: visibile solo con "Tutti" */
      const badge = card.querySelector('.project-tag');
      if (badge) {
        badge.style.display = current === 'Tutti' ? '' : 'none';
      }
    });
  });
...
/* ---- Footer social: Instagram + Facebook + altri dal JSON ---- */
export function renderFooterSocial(siteData) {
  ...
  /* Facebook: campo 'facebook' nel JSON oppure url generico */
